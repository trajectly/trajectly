...
""".strip(),
    ...
    assert "Next step: python -m trajectly record --auto" in enable_result.stdout
    ...
    assert record_result.exit_code == 0
    assert (tmp_path / ".trajectly" / "baselines" / "sample" / "v1" / "trace.jsonl").exists()
    assert (tmp_path / ".trajectly" / "baselines" / "sample" / "v1" / "fixtures.json").exists()
...
def test_baseline_versioning_commands_and_run_pinning(tmp_path: Path) -> None:
    script = tmp_path / "agent.py"
    _write_script(
        script,
        """
...
""".strip(),
    ...
    run_v1 = runner.invoke(app, ["run", str(spec), "--project-root", str(tmp_path), "--baseline", "v1"])
    assert run_v1.exit_code == 1

    create_v2 = runner.invoke(
        app,
    ...
    diff_v1_v2 = runner.invoke(
        app,
        ["baseline", "diff", "versioned", "v1", "v2", "--project-root", str(tmp_path), "--json"],
    )
    assert diff_v1_v2.exit_code == 0
    diff_payload = json.loads(diff_v1_v2.stdout)
    assert diff_payload["slug"] == "versioned"
    assert diff_payload["left"] == "v1"
    assert diff_payload["right"] == "v2"

    promote_v1 = runner.invoke(
        app,
        ["baseline", "promote", "v1", str(spec), "--project-root", str(tmp_path)],
    )
    assert promote_v1.exit_code == 0
    assert runner.invoke(app, ["run", str(spec), "--project-root", str(tmp_path)]).exit_code == 1

    promote_v2 = runner.invoke(
        app,
        ["baseline", "promote", "v2", str(spec), "--project-root", str(tmp_path)],
    )
    assert promote_v2.exit_code == 0
    assert runner.invoke(app, ["run", str(spec), "--project-root", str(tmp_path)]).exit_code == 0

    listed = runner.invoke(app, ["baseline", "list", "--project-root", str(tmp_path)])
    assert listed.exit_code == 0
    listed_payload = json.loads(listed.stdout)
    entries = {item["slug"]: item for item in listed_payload["specs"]}
    assert entries["versioned"]["promoted"] == "v2"
...
""".strip(),
    ...
    full_current = tmp_path / ".trajectly" / "current" / "repro.run.jsonl"
